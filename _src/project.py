...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        display_areas = "\n".join(
            f"{e['id'].rjust(5)} {e['name']}" for e in get_areas()
        )
        display_roles = "\n".join(
            f"{e['id'].rjust(5)} {e['name']}" for e in get_roles()
        )

        area_ids = validated_input(
            f"\nAvailable locations:\n{display_areas}\nPlease select ID(s) of your preferred work location(s):",
            get_areas(),
        )
        role_ids = validated_input(
            f"\nAvailable professional roles:\n{display_roles}\nPlease specify ID(s) of the job role(s) you are seeking:",
            get_roles(),
        )
    ...
    visualization_requested = input(
        "Would you like to generate a visualization in the form of charts to summarize the data? (y/n):"
    )
    if visualization_requested.lower() == "y":
        ...
        print(
            "If you want to analyze other vacancies, please rerun the program. Thank you."
        )
...
async def fetch_dictionaries(session):
    async with asyncio.TaskGroup() as tg:
        task1 = tg.create_task(
            fetch_one(session, "https://api.hh.ru/professional_roles?locale=EN")
        )
    ...
    print("Dictionaries were successfully fetched.")
...
async def fetch_vacancies(session, role_params, area_params):
    ...
    task = asyncio.create_task(
        fetch_one(session, "https://api.hh.ru/vacancies", vacancy_params)
    )
...
async def fetch_descriptions(session, vacancies):
    ...
    print(f"Fetching {total_urls} vacancy descriptions...")
...
def get_areas():
    ...
    return [
        {key: e[key] for key in keys} for e in areas if e["name"] != "Other regions"
    ]
...
# Counting skills by occurance and returning top 10
def count_skills(list):
    seen = []
    skills = {}
    for e in list:
        if e in seen:
